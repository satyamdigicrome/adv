...
@section('content')
    ...
    <!-- ===== HERO BANNER ===== -->
    <div
        style="position:relative;min-height:520px;display:flex;align-items:center;overflow:hidden;background:linear-gradient(135deg,#0f2044 0%,#1a3460 100%);">

        <!-- Background banner image with overlay -->
        @if ($attestation->banner_image)
            <div
                style="position:absolute;inset:0;background-image:url('{{ asset('storage/' . $attestation->banner_image) }}');background-size:cover;background-position:center;opacity:0.2;">
            </div>
        @endif

        <!-- Decorative elements -->
        <div
            style="position:absolute;top:-100px;right:-100px;width:500px;height:500px;border-radius:50%;background:rgba(201,168,76,0.06);pointer-events:none;">
        </div>
        <div
            style="position:absolute;bottom:-60px;left:30%;width:200px;height:200px;border-radius:50%;background:rgba(201,168,76,0.04);pointer-events:none;">
        </div>
                ...
                <div class="col-lg-7" data-aos="fade-right">
                    ...
                    <nav style="margin-bottom:20px;">
                        <ol
                            ...
                            <li><a href="{{ url('/') }}"
                                    style="color:rgba(255,255,255,0.5);text-decoration:none;transition:color 0.2s;"
                                    onmouseover="this.style.color='var(--gold)'"
                                    onmouseout="this.style.color='rgba(255,255,255,0.5)'">Home</a></li>
                            <li style="margin:0 4px;">/</li>
                            <li><a href="{{ route('attestations') }}"
                                    style="color:rgba(255,255,255,0.5);text-decoration:none;transition:color 0.2s;"
                                    onmouseover="this.style.color='var(--gold)'"
                                    onmouseout="this.style.color='rgba(255,255,255,0.5)'">Attestations</a></li>
                            <li style="margin:0 4px;">/</li>
                    ...
                    @if ($attestation->country)
                        <div
                            ...
                            <span
                                style="color:var(--gold);font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:1px;">{{ $attestation->country }}</span>
                        </div>
                    @endif

                    <h1
                        style="font-family:'Playfair Display',serif;color:#fff;font-size:clamp(28px,5vw,48px);font-weight:700;line-height:1.2;margin-bottom:18px;">
                        {{ $attestation->title }}
                    </h1>

                    @if ($attestation->short_description)
                        <p
                            style="color:rgba(255,255,255,0.72);font-size:15.5px;line-height:1.8;max-width:560px;margin-bottom:28px;">
                    ...
                    <div style="display:flex;gap:24px;margin-top:32px;flex-wrap:wrap;">
                        @foreach (['Govt. Approved', '100% Genuine', 'Fast Processing', 'Pan India'] as $badge)
                            <div style="display:flex;align-items:center;gap:6px;">
                                ...
                                <span
                                    style="font-size:12px;color:rgba(255,255,255,0.65);font-weight:500;">{{ $badge }}</span>
                            </div>
                        @endforeach
                    </div>
                </div>
                ...
                <div class="col-lg-5 d-none d-lg-block" data-aos="fade-left" data-aos-delay="100">
                    @if ($attestation->main_image)
                        <div style="position:relative;">
                            <img src="{{ asset('storage/' . $attestation->main_image) }}" alt="{{ $attestation->title }}"
                ...
                <div class="col-lg-8" data-aos="fade-right">
                    ...
                    @if ($attestation->long_description)
                        <div class="att-content"
                            style="background:#fff;border-radius:16px;padding:36px;box-shadow:0 2px 20px rgba(15,32,68,0.06);border:1px solid #edf1f8;margin-bottom:32px;word-wrap:break-word;overflow-wrap:break-word;">
                    ...
                    @if ($attestation->steps_image || ($attestation->steps && count($attestation->steps) > 0))
                        <div class="att-steps"
                            ...
                            @if ($attestation->steps_image)
                                <div style="margin-bottom:18px;">
                                    <img src="{{ $attestation->steps_image_url }}" alt="Work Steps"
                                        style="width:100%;height:auto;border-radius:12px;object-fit:cover;display:block;"
                                        onerror="this.style.display='none'" />
                                </div>
                            @endif

                            @if ($attestation->steps && count($attestation->steps) > 0)
                                <div style="margin-top:32px;">
                                    @foreach ($attestation->steps as $index => $step)
                            ...
                            @foreach ([['fas fa-check-double', '100% Authentic', 'All attestations done through official government channels.'], ['fas fa-bolt', 'Fast Processing', 'Express services available for urgent requirements.'], ['fas fa-headset', 'Dedicated Support', 'Assigned relationship manager for your documents.'], ['fas fa-rupee-sign', 'Transparent Pricing', 'No hidden charges. Get full quote upfront.'], ['fas fa-map-marked-alt', 'Pan India Service', 'Pickup and delivery across all major cities in India.'], ['fas fa-award', '15+ Years Exp.', 'Trusted by 10,000+ clients for document attestation.']] as $f)
                                <div class="col-md-6">
                                    <div style="display:flex;gap:14px;padding:16px;border-radius:12px;border:1px solid #edf1f8;background:#fafbfd;transition:all 0.2s;"
                                        ...
                                        <div
                                            style="width:40px;height:40px;background:rgba(201,168,76,0.1);border-radius:10px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                                            <i class="{{ $f[0] }}" style="color:var(--gold);font-size:15px;"></i>
                                        </div>
                                        <div>
                                            <div
                                                style="font-size:13.5px;font-weight:700;color:var(--primary);margin-bottom:3px;">
                                                {{ $f[1] }}</div>
                                            <div style="font-size:12.5px;color:#637082;line-height:1.5;">
                                                {{ $f[2] }}</div>
                                        </div>
                                    </div>
                                </div>
                ...
                <div class="col-lg-4">
                    ...
                    <div id="apply"
                        ...
                        <div style="display:flex;align-items:center;gap:12px;margin-bottom:6px;">
                            <div
                                style="width:40px;height:40px;background:rgba(201,168,76,0.15);border-radius:10px;display:flex;align-items:center;justify-content:center;">
                            ...
                            <div>
                                ...
                                <p style="color:rgba(255,255,255,0.55);font-size:12px;margin:0;">Free consultation included
                                </p>
                            </div>
                        </div>

                        <!-- Tab Toggle -->
                        <div style="display:flex;background:rgba(255,255,255,0.07);border-radius:10px;padding:4px;margin:18px 0 20px;"
                            id="formTabs">
                        ...
                        <form method="POST" action="{{ route('attestations.enquire', $attestation->slug) }}"
                            id="attestationForm">
                            @csrf
                    ...
                    <!-- Quick Contact Box -->
                    <div
                        style="background:#fff;border-radius:16px;padding:20px;box-shadow:0 2px 16px rgba(15,32,68,0.06);border:1px solid #edf1f8;margin-bottom:24px;">
                        <h6
                        ...
                        <a href="tel:+919354234462"
                            ...
                            <div
                                style="width:36px;height:36px;background:rgba(34,197,94,0.1);border-radius:50%;display:flex;align-items:center;justify-content:center;">
                        ...
                        <a href="https://wa.me/919354234462?text=Hi, I need {{ urlencode($attestation->title) }} assistance."
                            ...
                            <div
                                style="width:36px;height:36px;background:rgba(37,211,102,0.1);border-radius:50%;display:flex;align-items:center;justify-content:center;">
                    ...
                    <div
                        style="background:rgba(201,168,76,0.08);border:1.5px dashed rgba(201,168,76,0.4);border-radius:14px;padding:20px;margin-bottom:24px;text-align:center;">
                        <i class="fas fa-concierge-bell"
                            style="font-size:28px;color:var(--gold);margin-bottom:10px;display:block;"></i>
                        <p style="font-size:13px;color:var(--primary);font-weight:600;margin-bottom:14px;">Need other
                            document services?</p>
                        ...
                        <div
                            ...
                            @foreach ($services as $svc)
                                <a href="{{ route('services.show', $svc->slug) }}"
                                    ...
                                    <div
                                        style="width:36px;height:36px;background:rgba(15,32,68,0.06);border-radius:9px;display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                                        <i class="{{ $svc->icon ?? 'fas fa-file-alt' }}"
                                            style="color:var(--primary);font-size:13px;"></i>
                                    </div>
                                    <span
                                        style="font-size:13px;font-weight:600;color:var(--primary);line-height:1.3;">{{ $svc->title }}</span>
                    ...
                    @if ($related->isNotEmpty())
                        <div
                            style="background:#fff;border-radius:16px;padding:24px;box-shadow:0 2px 16px rgba(15,32,68,0.06);border:1px solid #edf1f8;">
                            <h6
                                style="font-size:13px;font-weight:700;color:var(--primary);margin-bottom:16px;text-transform:uppercase;letter-spacing:0.5px;">
                            ...
                            @foreach ($related as $rel)
                                <a href="{{ route('attestations.show', $rel->slug) }}"
                                    ...
                                    <div
                                        ...
                                        <i class="{{ $rel->icon ?? 'fas fa-certificate' }}"
                                            style="color:var(--gold);font-size:13px;"></i>
                                    </div>
                                    <div style="flex:1;min-width:0;">
                                        <span
                                            style="font-size:13px;font-weight:600;color:var(--primary);display:block;line-height:1.3;">{{ $rel->title }}</span>
                                        @if ($rel->country)
                                            <span style="font-size:11px;color:#8a99b0;">{{ $rel->country }}</span>
                                        @endif
                                    </div>
...
@push('styles')
    <style>
        ...
        .att-content table td {
            padding: 10px 16px;
            border-bottom: 1px solid #edf1f8;
            font-size: 14px;
        }

        .att-content table tr:hover td {
            background: #f8fafc;
        }

        .att-content blockquote {
        ...
        #attestationForm input::placeholder,
        #attestationForm textarea::placeholder {
