                                ...
                                @else
                                    <div
                                        ...
                                        <i class="{{ $att->icon ?? 'fas fa-certificate' }}"
                                            style="color:#c9a84c;font-size:18px;"></i>
                                    </div>
                                @endif
                            </td>
                            <td>
                                ...
                                @if ($att->short_description)
                                    <div style="font-size:12px;color:#8a99b0;margin-top:2px;">
                                        {{ Str::limit($att->short_description, 55) }}</div>
                                @endif
                            </td>
                            <td>
                                @if ($att->country)
                                    <span
                                        style="font-size:12px;background:rgba(201,168,76,0.1);color:#a8893a;padding:3px 10px;border-radius:50px;font-weight:600;">{{ $att->country }}</span>
                                @else
                                    <span style="color:#ccc;">—</span>
                                @endif
                            </td>
                            <td><code
                                    style="font-size:12px;color:#6366f1;">/attestation-services/{{ $att->slug }}</code>
                            </td>
                            <td>{{ $att->sort_order }}</td>
                            <td>
                                <form method="POST" action="{{ route('admin.attestations.toggle', $att) }}"
                                    style="display:inline;">
                                    @csrf @method('PATCH')
                                    <button type="submit"
                                        class="status-badge badge-{{ $att->is_active ? 'active' : 'inactive' }}"
                            ...
                            <td style="text-align:center;">
                                <div class="d-flex gap-2 justify-content-center">
                                    <a href="{{ route('attestations.show', $att->slug) }}" target="_blank"
                                        class="btn btn-sm"
                                        style="border:1px solid #e2e8f0;color:#555;border-radius:7px;padding:5px 10px;font-size:12px;"
                                        title="View">
                                        <i class="fas fa-eye"></i>
                                    </a>
                                    {{-- <form method="POST" action="{{ route('admin.attestations.duplicate', $att) }}"
                                        style="display:inline;">
                                        @csrf
                                        <button type="submit" class="btn btn-sm" title="Duplicate"
                                            style="background:rgba(34,197,94,0.1);color:#16a34a;border:none;border-radius:7px;padding:5px 10px;font-size:12px;">
                                            <i class="fas fa-copy"></i>
                                        </button>
                                    </form> --}}
                                    <a href="{{ route('admin.attestations.edit', $att) }}"
                                        class="btn btn-sm btn-primary-admin" style="padding:5px 10px;font-size:12px;"
                                        title="Edit">
                    ...
                    @empty
                        <tr>
                            <td colspan="9" class="text-center py-5 text-muted">
                                ...
                                No attestation pages yet. <a href="{{ route('admin.attestations.create') }}"
                                    style="color:var(--gold);">Add your first one</a>.
                            </td>
                        </tr>
                    @endforelse
                </tbody>
