...
<div class="page-header">
    <div>
        ...
        <div class="page-breadcrumb"><a href="{{ route('admin.dashboard') }}">Dashboard</a> / <a href="{{ route('admin.blogs.index') }}">Blogs</a> / Edit</div>
    </div>
    <div class="d-flex gap-2">
        <a href="{{ route('blog.show', $blog->slug) }}" target="_blank" class="btn btn-sm" style="border:1.5px solid #e2e8f0;font-size:13px;font-weight:600;color:#16a34a;border-radius:8px;padding:8px 18px;">
    ...
    <div class="col-lg-8">
        ...
        <div class="form-card mb-4">
            ...
            <div class="mb-4">
                ...
                <textarea name="short_description" class="form-control @error('short_description') is-invalid @enderror"
                          rows="3" placeholder="Brief summary shown on blog listing page (max 500 chars)" maxlength="500" required>{{ old('short_description', $blog->short_description) }}</textarea>
                @error('short_description')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>
            <div>
                <label class="form-label">Full Content <span class="text-danger">*</span></label>
                <textarea name="long_description" id="blogContent" class="form-control">{{ old('long_description', $blog->long_description) }}</textarea>
            </div>
        </div>
        ...
        <div class="form-card">
            ...
            <div class="mb-3">
                ...
                <input type="text" name="meta_title" class="form-control" value="{{ old('meta_title', $blog->meta_title) }}" placeholder="Leave blank to use post title">
            </div>
            <div class="mb-3">
                <label class="form-label">Meta Description</label>
                <textarea name="meta_description" class="form-control" rows="2" placeholder="SEO description">{{ old('meta_description', $blog->meta_description) }}</textarea>
            </div>
            <div>
                <label class="form-label">Meta Keywords</label>
                <input type="text" name="meta_keywords" class="form-control" value="{{ old('meta_keywords', $blog->meta_keywords) }}" placeholder="keyword1, keyword2">
            </div>
        </div>
    </div>
    ...
    <div class="col-lg-4">
        ...
        <div class="form-card mb-4">
            ...
            <div class="mb-3 d-flex align-items-center justify-content-between p-3 rounded-2" style="background:#f8fafc;border:1.5px solid #e2e8f0;">
                ...
                <div class="form-check form-switch mb-0">
                    <input class="form-check-input" type="checkbox" name="is_active" {{ old('is_active', $blog->is_active) ? 'checked' : '' }} style="width:42px;height:22px;cursor:pointer;">
                </div>
            </div>
            <div class="mb-3">
            ...
            <div class="mb-4">
                ...
                <input type="text" name="author" class="form-control" value="{{ old('author', $blog->author) }}" placeholder="Author name">
            </div>
            <div class="mb-4">
            ...
            <div class="d-grid gap-2">
                <button type="submit" class="btn btn-gold-admin"><i class="fas fa-save me-2"></i> Update Post</button>
            ...
            <!-- Danger Zone -->
            <button type="button"
                    onclick="adminDelete('{{ route('admin.blogs.destroy', $blog) }}', 'Delete this blog post permanently? This cannot be undone.')"
                    class="btn w-100"
        ...
        <div class="form-card">
            ...
            <div id="imgPreviewWrap" style="margin-bottom:12px;border-radius:10px;overflow:hidden;aspect-ratio:16/9;background:#f8fafc;border:2px dashed #e2e8f0;display:flex;align-items:center;justify-content:center;cursor:pointer;" onclick="document.getElementById('imgInput').click()">
                @if($blog->image)
                    <img id="imgPreview" src="{{ asset('storage/'.$blog->image) }}" alt="" style="width:100%;height:100%;object-fit:cover;">
                    <div id="imgPlaceholder" style="display:none;text-align:center;color:#bbb;">
                        <i class="fas fa-cloud-upload-alt fa-2x mb-2 d-block"></i>
                        <span style="font-size:12px;">Click to change image<br>(16:9 recommended)</span>
                    </div>
                @else
            ...
            <button type="button" onclick="document.getElementById('imgInput').click()" class="btn w-100" style="border:1.5px solid #e2e8f0;border-radius:8px;padding:9px;font-size:13px;font-weight:600;color:#555;">
                <i class="fas fa-upload me-2"></i> {{ $blog->image ? 'Change Image' : 'Choose Image' }}
            </button>
            @if($blog->image)
            <div class="mt-2" style="font-size:11.5px;color:#8a99b0;text-align:center;">Leave blank to keep current image</div>
            @endif
        </div>
    </div>
</div>
