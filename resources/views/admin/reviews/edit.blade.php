...
<div class="page-header">
    <div>
        <h4>Edit Review</h4>
        <div class="page-breadcrumb"><a href="{{ route('admin.dashboard') }}">Dashboard</a> / <a href="{{ route('admin.reviews.index') }}">Reviews</a> / Edit</div>
    </div>
        ...
        <div class="form-card">
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="reviewer_name" class="form-control @error('reviewer_name') is-invalid @enderror"
                           value="{{ old('reviewer_name', $review->reviewer_name) }}" placeholder="e.g. Rahul Sharma" required>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="reviewer_location" class="form-control @error('reviewer_location') is-invalid @enderror"
                           value="{{ old('reviewer_location', $review->reviewer_location) }}" placeholder="e.g. Delhi, India">
            ...
            <div class="mt-4">
                ...
                <div class="d-flex gap-3 flex-wrap mt-1">
                    @for($i = 1; $i <= 5; $i++)
                    @php $selected = old('rating', $review->rating) == $i; @endphp
                    <label style="cursor:pointer;">
                        <input type="radio" name="rating" value="{{ $i }}" {{ $selected ? 'checked' : '' }} style="display:none;" class="rating-radio">
                        <div class="rating-option p-3 rounded-2 text-center"
                             style="border:2px solid {{ $selected ? 'var(--gold)' : '#e2e8f0' }};background:{{ $selected ? 'rgba(201,168,76,0.1)' : '#f8fafc' }};min-width:80px;transition:all 0.2s;">
            ...
            <div class="mt-4">
                ...
                <textarea name="review_text" class="form-control @error('review_text') is-invalid @enderror"
                          rows="5" placeholder="Enter the customer review text here..." maxlength="2000" required>{{ old('review_text', $review->review_text) }}</textarea>
    ...
    <div class="col-lg-4">
        <div class="form-card mb-4">
            ...
            <div class="mb-3">
                ...
                <input type="number" name="sort_order" class="form-control" value="{{ old('sort_order', $review->sort_order) }}" min="0">
            ...
            <div class="mb-4 d-flex align-items-center justify-content-between p-3 rounded-2" style="background:#f8fafc;border:1.5px solid #e2e8f0;">
                ...
                <div class="form-check form-switch mb-0">
                    <input class="form-check-input" type="checkbox" name="is_active" {{ old('is_active', $review->is_active) ? 'checked' : '' }} style="width:42px;height:22px;cursor:pointer;">
                </div>
            </div>
            <div class="section-divider"></div>
            <div class="d-grid gap-2">
                <button type="submit" class="btn btn-gold-admin"><i class="fas fa-save me-2"></i> Update Review</button>
                <a href="{{ route('admin.reviews.index') }}" class="btn text-center" style="border:1.5px solid #e2e8f0;font-size:13.5px;border-radius:8px;padding:10px;font-weight:600;color:#555;">Cancel</a>
            </div>
            <div class="section-divider"></div>
            <button type="button"
                    onclick="adminDelete('{{ route('admin.reviews.destroy', $review) }}', 'Delete this review permanently? This cannot be undone.')"
                    class="btn w-100"
                    style="border:1.5px solid rgba(239,68,68,0.3);color:#dc2626;border-radius:8px;padding:9px;font-size:13px;font-weight:600;">
        ...
        <div class="form-card">
            ...
            <div style="background:#f8fafc;border-radius:10px;padding:16px;border:1px solid #e2e8f0;">
                <div class="d-flex align-items-center gap-2 mb-3">
                    <div id="prevAvatar" style="width:42px;height:42px;border-radius:50%;background:linear-gradient(135deg,var(--primary),#1a3460);display:flex;align-items:center;justify-content:center;color:var(--gold);font-weight:700;font-size:14px;">{{ $review->initials }}</div>
                    <div>
                        <div id="prevName" style="font-weight:700;font-size:13.5px;color:var(--primary);">{{ $review->reviewer_name }}</div>
                        <div id="prevLocation" style="font-size:11.5px;color:#8a99b0;">{{ $review->reviewer_location ? '📍 '.$review->reviewer_location : '' }}</div>
                    </div>
                </div>
                <div id="prevStars" style="color:#f5b301;font-size:14px;margin-bottom:8px;">
                    @for($i = 1; $i <= 5; $i++)
                        <i class="fas fa-star" style="color:{{ $i <= $review->rating ? '#f5b301' : '#ddd' }}"></i>
                    @endfor
                </div>
                <div id="prevText" style="font-size:13px;color:#637082;line-height:1.7;font-style:italic;">"{{ $review->review_text }}"</div>
            </div>
        </div>
    </div>
</div>
...
function updatePreview() {
    ...
    var text     = document.querySelector('[name="review_text"]').value || '...';
